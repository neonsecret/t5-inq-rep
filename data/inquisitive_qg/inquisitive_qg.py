...
class InquisitiveQgConfig(nlp.BuilderConfig):
    ...
    def __init__(self, qg_format="highlight", **kwrags):
        """BuilderConfig for INQUISITIVE.
...
class InquisitiveQg(nlp.GeneratorBasedBuilder):
    """Inquisitive Question Generation for High Level Text Comprehension"""
    ...
    def _info(self):
        return nlp.DatasetInfo(
            ...
            features=nlp.Features(
                {
                    # "id": nlp.Value("int32"),
                    # "article_id": nlp.Value("int32"),
                    # "article": nlp.Value("string"),
                    # "sentence_id": nlp.Value("int32"),
                    # "sentence": nlp.Value("string"),
                    # "span": nlp.Value("string"),
                    # "question": nlp.Value("string"),
                    # "span_start_position": nlp.Value("int32"),
                    # "span_end_position": nlp.Value("int32"),
    ...
    def _generate_examples(self, articles_dir, questions_file, article_ids, files):
        ...
        with open(questions_file, encoding="utf-8") as f:
            ...
            for i, row in enumerate(rows):
                ...
                articles = {k[-16:].replace("\\", "/") if True else k: v for k, v in articles.items()}
                article = articles[article_path]
                id_ = str(questions_counter)

                # task = "qg"
                # task = random.choice(['qa', 'qg', 'ans_ext', 'e2e_qg'])
                # if task == "ans_ext":
                #     example = {
                #         "source_text": "extract answers: " + article,
                #         "target_text": cols[2] + "{sep_token}",
                #         "task": "ans_ext"
                #     }
                # elif task == "e2e_qg":
                #     example = {
                #         "source_text": "generate questions: " + article,
                #         "target_text": cols[4] + "{sep_token}",
                #         "task": "e2e_qg"
                #     }
                # elif task == "qa":
                #     example = {
                #         "source_text": "question: " + cols[4] + " context: " + article,
                #         "target_text": cols[2],
                #         "task": "qa"
                #     }
                # elif task == "qg":
